...
# 读取数据
with open('fb_optimized_data_structure_update.json', 'r', encoding='utf-8') as f:
    opt_data = json.load(f)
with open('fb_fetch_inf_update.json', 'r', encoding='utf-8') as f:
...
for fid, fname in target_rec['available_fields'].items():
    ...
    if match_key:
        v = fb_data[match_key]
        # Views 字段特殊处理，直接写入明细或汇总到 Views 字段
        if fname == 'Views':
            if isinstance(v, list):
                result_data[fname] = v
            elif isinstance(v, dict) and 'graphData' in v:
            ...
            result_data[fname] = v['count']
        elif isinstance(v, dict) and 'list' in v:
            result_data[fname] = v['list']
        elif isinstance(v, dict) and 'graphData' in v:
            result_data[fname] = v['graphData']
        elif isinstance(v, dict) and 'overview' in v:
...
# 保存
with open('fb_optimized_data_structure_update_filled.json', 'w', encoding='utf-8') as f:
    json.dump(opt_data, f, ensure_ascii=False, indent=2)

print('字段优化写入完成，已生成 fb_optimized_data_structure_update_filled.json') 