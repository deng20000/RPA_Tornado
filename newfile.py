            ...
            if '订单状态' in df.columns:
                ...
                if len(filtered_df) > 0:
                    ...
                    try:
                        ...
                        warehouse_mapping = dict(zip(filtered_df['用户订单号'], filtered_df['仓库名称']))
...
def process_semi_warehouse():
    ...
    try:
        ...
        df['业务类型'] = df['业务类型'].astype(str).apply(lambda x: x.strip())
        df_filtered = df[df['业务类型'].str.contains('TOC', case=False, na=False)]
        ...
        if len(df_filtered) > 0:
            ...
            try:
                df_sheet1 = pd.read_excel(output_file, sheet_name='sheet1', dtype=str)
                mask_empty = (df_sheet1['发货仓库'].isna()) | (df_sheet1['发货仓库'] == '')
                warehouse_mapping = dict(zip(df_filtered['交易主单'], df_filtered['仓库名称']))
...
def create_check_sheet():
    try:
        ...
        # 处理金额列，确保保留两位小数
        df_check['订单金额'] = df_backend['订单金额'].apply(
            lambda x: '{:.2f}'.format(float(x)) if pd.notnull(x) and str(x).strip() != '' else '0.00'
        )
        df_check['物流费用'] = df_backend['物流费用'].apply(
            lambda x: '{:.2f}'.format(float(x)) if pd.notnull(x) and str(x).strip() != '' else '0.00'
        )
